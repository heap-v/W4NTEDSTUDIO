...
local Highlight =
	loadstring(
		game:HttpGet("https://github.com/heap-v/Wanted/main/W4NTEDVIEW2.lua")
	)()
...
function newRemote(type, name, args, remote, function_info, blocked, src, returnValue)
	...
	schedule(function()
		...
		if blocked then
			logs[#logs].GenScript = "-- THIS REMOTE WAS PREVENTED FROM FIRING THE SERVER BY W4NTED VIEW\n\n"
				.. logs[#logs].GenScript
		end
	end)
...
if not _G.SimpleSpyExecuted then
	local succeeded, err = pcall(function()
		if not RunService:IsClient() then
			error("W4NTED VIEW cannot run on the server!")
		end
		if
			not hookfunction
		...
		then
			...
			shutdown()
			error(
				"This environment does not support method hooks!\n(Your exploit is not capable of running W4NTED VIEW)\nMissing: "
	...
	if not succeeded then
		warn(
			"A fatal error has occured, W4NTED VIEW was unable to launch properly.\nPlease join https://dsc.gg/w4nted and message heap:\n\n"
				.. tostring(err)
		)
		SimpleSpy2:Destroy()
		hookfunction(remoteEvent.FireServer, originalEvent)
...
newButton("Function Info", function()
	...
	if selected then
		if selected.Function then
			codebox:setRaw(
				"-- Calling function info\n-- Generated by the W4NTED VIEW serializer\n\n" .. tostring(selected.Function)
			)
		end
		TextLabel.Text = "Done! Function info generated by the W4NTED VIEW Serializer."
	end
end)
...
newButton("Exclude (i)", function()
	return "Click to exclude this Remote.\nExcluding a remote makes W4NTED VIEW ignore it, but it will continue to be usable."
end, function()
	if selected then
...
newButton("Block (n)", function()
	return "Click to stop remotes with this name from firing.\nBlocking a remote won't remove it from W4NTED VIEW logs, but it will not continue to fire the server."
end, function()
	if selected then
...
newButton("CallingScript", function()
	return string.format(
		"[%s] [UNSAFE] Uses 'getcallingscript' to get calling script for Decompile and GetScript. Much more reliable, but opens up W4NTED VIEW to detection and/or instability.",
